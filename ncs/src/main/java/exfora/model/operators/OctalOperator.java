...
public class OctalOperator {
    ...
    public static String plus(String x, String y) {
        ...
        for (int i = maxLength - 1; i >= 0; i--) {
            int digit1 = Character.digit(x.charAt(i), 8);
            int digit2 = Character.digit(y.charAt(i), 8);
            int sum = digit1 + digit2 + carry;
            result.append(Integer.toOctalString(sum % 8));
            carry = sum / 8;
        }

        if (carry != 0) {
            result.append(Integer.toOctalString(carry));
        }
    ...
    public static String minus(String x, String y) {
        ...
        for (int i = maxLength - 1; i >= 0; i--) {
            ...
            result.append(Integer.toOctalString(diff));
        }
    ...
    public static String multiply(String x, String y) {
        ...
        for (int i = x.length() - 1; i >= 0; i--) {
            for (int j = y.length() - 1; j >= 0; j--) {
                int digit1 = Character.digit(x.charAt(i), 8);
                int digit2 = Character.digit(y.charAt(j), 8);
        ...
        for (int digit : result) {
            resultStr.append(Integer.toOctalString(digit));
        }
    ...
    public static String divide(String x, String y) {
        ...
        for (int i = 0; i < x.length(); i++) {
            ...
            if (compareOctal(remainder.toString(), y) >= 0) {
                quotient.append('1');
    ...
    private static int compareOctal(String x, String y) {
